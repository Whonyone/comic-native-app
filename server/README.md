...
작품 정보 수정. 모든 필드 선택 사항 (변경하려는 필드만 전송).
...
작품 삭제. 연관된 모든 회차(percomics), 회차 이미지(percomic_images), 업로드 파일 폴더도 함께 삭제됩니다.
...
회차 수정. 이미지를 새로 업로드하면 기존 이미지 전체가 교체됩니다. 파일 없이 `existingImagePaths`만 전송하면 순서/삭제만 반영됩니다.
...
회차 삭제. 연관된 이미지 레코드(percomic_images)와 회차 업로드 폴더도 함께 삭제됩니다.
